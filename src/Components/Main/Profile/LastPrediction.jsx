...
export default function LastPrediction() {
    ...
    const { bet } = useSelector(state => state.user);
    ...
    const isScored = lastPrediction.breakdown || lastPrediction.standardBreakdown;
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900 flex flex-col">
            ...
            <div className="flex-1 max-w-4xl mx-auto w-full p-4 md:p-8 mt-6">
                ...
                <div className="bg-gradient-to-br from-gray-800/50 to-black/50 backdrop-blur-md rounded-2xl shadow-2xl border border-red-900/30 p-6 mb-8 relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        <div className="flex items-center space-x-3 mb-2">
                            ...
                            {isScored && <span className="bg-green-600 text-white text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider shadow-lg">Scored</span>}
                        </div>
                        ...
                        <p className="text-xl text-gray-400 font-medium flex items-center">
                            <span className="mr-2">🏁</span> {isScored ? 'Race Results & Score' : 'Latest Prediction'}
                        </p>
                    </div>
                </div>
                ...
                <div className="bg-gradient-to-br from-gray-800/50 to-black/50 backdrop-blur-md rounded-2xl shadow-2xl border border-red-900/30 overflow-hidden">
                    <div className="bg-gradient-to-r from-red-900/30 to-orange-900/30 border-b border-red-900/30 px-6 py-4 flex justify-between items-center">
                        <h3 className="font-black text-white text-lg italic flex items-center">
                            <span className="text-2xl mr-2">{isScored ? '📊' : '🎯'}</span> {isScored ? 'Scoring Breakdown' : 'Your Top 10 Prediction'}
                        </h3>
                        {/* If multiple leagues exist, we show the standard shadow score here */}
                        {isScored && lastPrediction.standardScore !== undefined && (
                            <div className="bg-black/40 px-3 py-1 rounded-full border border-red-500/30">
                                <span className="text-xs text-gray-400 uppercase tracking-widest mr-2">Global Score:</span>
                                <span className="text-xl font-black text-red-500">+{lastPrediction.standardScore}</span>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="p-6">
                        {isScored ? (
                            <ScoringBreakdown
                                ...
                                breakdown={lastPrediction.standardBreakdown || lastPrediction.breakdown}
                                officialResults={lastPrediction.officialResults}
                                totalScore={lastPrediction.standardScore}
                            />
                        ) : (
                            /* Original simple list for unscored predictions */
